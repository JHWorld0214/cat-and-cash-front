...
import { View, Text, FlatList, SafeAreaView, StatusBar, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
export default function ChatScreen() {
    ...
    const sendToServer = async (text: string): Promise<string[]> => {
        // TODO: axios/post 요청 후 응답 리스트 반환
        return ['안녕!', '무슨 도움이 필요해요?'];
    };

    const { messages, input, isBotTyping, onInputChange } = useChat(sendToServer);
                ...
                <View style={{ marginBottom: isLastBot ? 12 : 2 }}>
                    ...
                    <View style={{ flexDirection:'row', alignItems:'flex-start' }}>
                        ...
                        <View style={{ maxWidth:'75%' }}>
                            ...
                            {isLastBot && <Text style={{ fontSize:12, color:'#999', marginTop:2, alignSelf:'flex-end', marginRight:6 }}>{/* timestamp */}</Text>}
                        </View>
                    </View>
                </View>
            );
        ...
        <SafeAreaView style={{ flex:1, backgroundColor:'#F4F3FF' }}>
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View style={{ flex:1, paddingTop:60, paddingHorizontal:16, paddingBottom: insets.bottom }}>
                    ...
                    <ChatInput value={input} onChangeText={onInputChange}/>
                </View>
