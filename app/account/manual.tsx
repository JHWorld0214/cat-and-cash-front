...
export default function ManualEntryScreen() {
    ...
    useEffect(() => {
        const fetchNyang = async () => {
            ...
            if (!isNaN(numeric) && numeric > 0) {
                const result = await calDecAmount(numeric);
                setNyang(result);
            } else {
                setNyang(null);
            }
    ...
    const formatDate = (d: Date) =>
        `${d.getFullYear()}년 ${d.getMonth() + 1}월 ${d.getDate()}일 오전 ${d.getHours()}:${d.getMinutes().toString().padStart(2, '0')}`;
